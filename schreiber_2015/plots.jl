...
In al cases ``x = \log(\mathrm{M_\star \, / \, M_\odot})`` and ``y = \log(\mathrm{SFR \, / \, M_\odot \, yr^{-1}})``
"""
...
``\log_{10}(\mathrm{SFR \, / \, M_\odot \, yr^{-1}}) = m - m_0 + a_0 \, r - a_1 \, [\max(0, m - m_1 - a_2 \, r)]^2``

where ``r = \log_{10}(1 + z)`` and ``m = \log_{10}(\mathrm{M_\star \, / \, 10^9 \, M_\odot})``
"""

# ╔═╡ fd58dcc1-092d-455e-9aa5-e9ce61e8416a
fit = DataFrame(
	parameter = ["m0", "m1", "a0", "a1", "a2"],
	values    = [0.5 ± 0.07, 0.36 ± 0., 1.5 ± 0.15, 0.3 ± 0.08, 2.5 ± 0.6],
)

# ╔═╡ 105261ea-22b1-4506-b6c5-9a44ed636921
let
	fit_func = (m, r) -> Measurements.value(
		m - fit[1, :values] + fit[3, :values] * r -  fit[4, :values] * (max(0, m - fit[2, :values] - fit[5, :values] * r))^2
	)
	...
	ax = Axis(
		f[1,1],
		xlabel=L"\log(\mathrm{M_\star / \, 10^9 \, M_\odot})",
...
manifest_format = "2.0"
project_hash = "5fba1c64e512b8c2bfa4a4ed0ab50a153b8b18b9"
