...
begin
	table_lines = readlines("./data/table4.txt")
	extrema(length.(table_lines))
	col_range = [
		1:11,
		12:13,
		14:19,
	...
	logSFR = parse.(Float64, data[:, 8])
	logMHI = parse.(Float64, replace(data[:, end], "" => "nan"))
end;
...
let
	...
	scatter!(ax, logMs, logSFR; markersize=10, color=:red)

	f
end

# ╔═╡ 51cbbf9f-d1f4-4f33-880b-501af9ff1cd3
let
	set_theme!(theme_black())
	...
	ax = Axis(
		f[1,1],
		xlabel=L"\log_{10}(\mathrm{M_{HI} / M_\odot})",
		ylabel=L"\log_{10}(\mathrm{SFR / M_\odot \, yr^{-1}})",
		title=L"\mathrm{SFR \,\, vs. \,\, Atomic \,\, mass}",
		titlesize=30,
		xlabelsize=28,
		ylabelsize=28,
		xticklabelsize=20,
	...
	scatter!(ax, logMHI, logSFR; markersize=10, color=:red)
