...
using CairoMakie, CSV, DataFrames, LaTeXStrings, Images, Measurements
...
# ╔═╡ 643c7619-0dff-4b09-af48-f94e58cce39c
load("./data/fig_13.png")
...
let
	...
	ax = CairoMakie.Axis(
		...
		xticklabelsize=20,
		yticklabelsize=20,
		limits=(nothing, nothing, -3.8, -1.2),
		xticks=0:7,
	)
	...
	logDTS(z, M) = A + η0 * (M - 10.0) + η1 * log10(1 + z)

	zs = collect(range(0.0, 7.5, 100))

	for (m, color) in zip([8.0, 9.0, 10.0], Makie.wong_colors()) 
		...
		band!(ax, zs, low_vals, high_vals; color, alpha=0.5)
		lines!(ax, zs, mean_vals; color, label=L"M_\star = %$(m)")
	...
	data_mean = "./data/mean_vals_fig_13.csv"
	data_low  = "./data/low_vals_fig_13.csv"
	data_high = "./data/high_vals_fig_13.csv"
	
	df_mean = CSV.read(data_mean, DataFrame)
	df_low  = CSV.read(data_low, DataFrame)
	df_high = CSV.read(data_high, DataFrame)
	...
	errorbars!(ax, z, dtg, dtg .- dtg_low, dtg_high .- dtg; color=:white)
	scatter!(ax, z, dtg; color=:white, label=L"\text{Measurements}")

	axislegend(ax; position=:rb, labelsize=18, framevisible=false)
...
manifest_format = "2.0"
project_hash = "b65c891cf62a77c31783e4b3b8e85b0cce5dfb55"
...
git-tree-sha1 = "15bf6e229ce090dbaa96d4d73ed72ce35cce864b"
...
git-tree-sha1 = "05f41085f3ec1438cbd85f95f194961bddb41fc1"
