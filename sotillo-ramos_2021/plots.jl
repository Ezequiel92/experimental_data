...
using CairoMakie, LaTeXStrings, DataFrames, Measurements, CSV
...
In all cases ``x = \log(M_\star / M_\odot)``
...
begin
	raw_data = CSV.read(
		"./data/clean_table_03.csv",
		DataFrame,
		header = ["Relation", "a", "b", "c"],
	)
	fit = DataFrame(
		Relation = raw_data[!, "Relation"],
		a        = measurement.(raw_data[!, "a"]),
		b        = measurement.(raw_data[!, "b"]),
		c        = [measurement.(raw_data[1, "c"])..., missing, missing],
	)
end

# ╔═╡ e68c931e-5569-4135-ae25-f8f0235f3d40
begin
	MZ    = x -> Measurements.value(fit[1, :a] + fit[1, :b] * x + fit[1, :c] * x^2)
	MSFR  = x -> Measurements.value(fit[2, :a] + fit[2, :b] * x)
	MsSFR = x -> Measurements.value(fit[3, :a] + fit[3, :b] * x)
end;
...
let
	...
	lines!(ax, 8..12, MSFR, color=:red)

	f
end

# ╔═╡ 4bb5e8c2-7291-47b5-a68e-e98ed8d694fb
let
	set_theme!(theme_black())
...
manifest_format = "2.0"
project_hash = "415f9ab86546199c670914d6dadbab1f01520842"
