...
using CSV, CairoMakie, LaTeXStrings, DataFrames, Measurements
...
begin
	data = dropmissing(raw_data, ["lgMstar", "SFR"])

	redshift = data[:, "z"]
	mass     = data[:, "lgMstar"]
	mass_c18 = data[:, "lgMstar_C18"]
	mass_p84 = data[:, "lgMstar_p84"]
	mass_p16 = data[:, "lgMstar_p16"]
	sfr      = data[:, "SFR"]
	sfr_err  = data[:, "e_SFR"]
	mHI      = data[:, "MHI"]
	mHI_err  = data[:, "e_MHI"]
...
let
	...
	scatter!(ax, mass, sfr, color=:red, markersize=5)
...
let
	...
	errorbars!(
		ax,
		redshift, mass,
		mass .- mass_p16, mass_p84 .- mass,
		color=:blue,
		whiskerwidth=8,
	)
	scatter!(ax, redshift, mass, color=:red, markersize=4)
