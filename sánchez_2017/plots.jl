...
begin
	raw_data, header = readdlm("./data/raw_table_B1.txt", header=true)
	data = identity.(DataFrame(raw_data, vec(header)))
...
let
	...
	log_mass_err = data[!, "e_log_mass"]
	log_sfr_err  = data[!, "e_log_sfr"]
	...
	scatter!(ax, log_mass, log_sfr, color=:blue, markersize=5)
	errorbars!(
		ax,
		log_mass,
		log_sfr,
...
let
	...
	calibrators = vec(header)[6:2:end]
	errors      = vec(header)[7:2:end]
	...
	for (cal, err, color) in zip(calibrators, errors, colors)
		oh     = data[!, cal]
		oh_err = data[!, err]

		scatter!(ax, log_mass, oh; color, markersize=5, label=cal[4:end])
		errorbars!(
			ax,
			log_mass,
			oh,
...
# ╔═╡ 315d75e5-ef5c-48e0-9fbf-a58accf57411
let

	μ = data[!, "log_mass"] .- 0.32 .* data[!, "log_sfr"]

	f = Figure()

	ax = Axis(
		f[1,1],
		xlabel=L"\mu = \log(\mathrm{M_\star / M_\odot}) - 0.32 \, \log(\mathrm{SFR / M_\odot \, yr^{-1}})",
		ylabel=L"12 + \log(\mathrm{O / H})",
		title=L"\mathrm{Metallicity \,\, vs.} \,\, \mu",
		titlesize=30,
		xlabelsize=28,
		ylabelsize=28,
		xticklabelsize=20,
	...
	for (cal, color) in zip(calibrators, colors)
		oh = data[!, cal]
		scatter!(ax, μ, oh; color, markersize=5, label=cal[4:end])
	end
...
manifest_format = "2.0"
project_hash = "5d41993d079d148a7de8fa02be672bb0e2e0da78"
