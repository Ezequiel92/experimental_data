...
using CairoMakie, DataFrames, DelimitedFiles, Unitful, UnitfulAstro

# ╔═╡ 5a95fa6c-6380-43ff-85bb-ead10e51c482
md"""
# [Haardt et al. (2012)](https://doi.org/10.1088/0004-637X/746/2/125)

### [Data](https://iopscience.iop.org/0004-637X/746/2/125/suppdata/apj412591t3_ascii.txt?doi=10.1088/0004-637X/746/2/125)
"""

# ╔═╡ b509f706-11a8-437f-ae99-42f259edea73
raw_data = readdlm("./data/apj412591t3_ascii.txt", skipstart=6)

# ╔═╡ 3f0b4a53-e763-407a-8203-9473958490dd
let
	z    = raw_data[:, 1]
	ΓUVB = ustrip.(u"Myr^-1", raw_data[:, 2] .* u"s^-1")

	data = DataFrame(z=z, rate=ΓUVB)

	writedlm("./output_table.txt", eachrow(data), ' ')
end
...
manifest_format = "2.0"
project_hash = "98d3b7afa632922eb49c91fc46ed8805714980ff"
