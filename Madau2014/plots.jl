...
# ╔═╡ 5a35809e-0fbb-4dc9-a9cc-b23cd119a8ff
function madau2014(z::Float64)::Float64
	
	z1 = 1 + z

	# Returns the CSFRD (cosmis star-formation rate density) in M⊙ yr^-1 Mpc^-3
	0.015 * z1^2.7 / (1 + (z1/2.9)^5.6)
...
let
	...
	ax = Axis(
		...
		ylabel = L"\mathrm{CSFRD} \, / \, \mathrm{M_{\!\odot} \, yr^{-1} \, Mpc^{-3}}", 
		xreversed = true,
		xscale = log10,
		yscale = log10,
		xlabelsize = 22,
	...
	lines!(ax, 0.8..8, madau2014, markersize = 6, color = :red)
